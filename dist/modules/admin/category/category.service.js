...
const category_vaidation_1 = require("./category.vaidation");
const crypto_1 = require("crypto");
// ============= CATEGORY SERVICES =============
const createCategory = async (data) => {
    const validatedData = category_vaidation_1.createCategorySchema.parse(data);
...
const updateCategory = async (categoryId, data) => {
    const validatedData = category_vaidation_1.updateCategorySchema.parse(data);
    ...
    if (validatedData.name && validatedData.name !== existingCategory.name) {
        const duplicateCategory = await prisma_1.prisma.category.findUnique({
            where: { name: validatedData.name }
        });
...
const createSubject = async (data) => {
    const validatedData = category_vaidation_1.createSubjectSchema.parse(data);
    // Check if category exists
    const category = await prisma_1.prisma.category.findUnique({
...
const updateSubject = async (subjectId, data) => {
    const validatedData = category_vaidation_1.updateSubjectSchema.parse(data);
    ...
    if (validatedData.name || validatedData.categoryId) {
        ...
        const duplicateSubject = await prisma_1.prisma.subject.findFirst({
            where: {
                name: targetName,
