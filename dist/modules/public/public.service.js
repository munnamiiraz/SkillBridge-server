...
class PublicService {
    static async searchTutors(filters, paginationOptions) {
        ...
        try {
            const [tutors, total] = await Promise.all([
                prisma_1.prisma.tutor_profile.findMany({
                    where: {
                        AND: [
                            whereClause,
                            {
                ...
                prisma_1.prisma.tutor_profile.count({ where: whereClause })
            ]);
    ...
    static async getTutorById(id) {
        const tutor = await prisma_1.prisma.tutor_profile.findFirst({
            where: {
                AND: [
                    {
                        OR: [
        ...
        if (!tutor) {
            const availableTutors = await prisma_1.prisma.tutor_profile.findMany({
    ...
    static async getFeaturedTutors(paginationOptions) {
        const [tutors, total] = await Promise.all([
            ...
            prisma_1.prisma.tutor_profile.count({
                where: {
                    isFeatured: true,
                    isAvailable: true,
                    user: {
    ...
    static async getTutorAvailability(tutorId, weekStartDate) {
        const tutorProfile = await prisma_1.prisma.tutor_profile.findFirst({
            where: {
                OR: [
                    { id: tutorId },
                    { userId: tutorId }
