...
const updateProfile = async (userId, data) => {
    const validatedData = student_validation_1.updateProfileSchema.parse(data);
    const updateData = {};
...
const createReview = async (studentId, data) => {
    // Validate input
    const validatedData = student_validation_1.createReviewSchema.parse(data);
...
const createBooking = async (studentId, data) => {
    try {
        const validatedData = booking_validation_1.createBookingSchema.parse(data);
        ...
        const availabilitySlot = await prisma_1.prisma.availability_slot.findFirst({
            where: {
                tutorProfileId: validatedData.tutorProfileId,
...
const getBookings = async (studentId, options) => {
...
const getReviewableBookings = async (studentId, options) => {
    const paginationHelper = (0, paginationSortingHelper_1.default)({
        page: options.page,
        limit: options.limit,
        sortBy: "scheduledAt",
...
const getReviews = async (studentId, options) => {
    const paginationHelper = (0, paginationSortingHelper_1.default)({
        page: options.page,
        limit: options.limit,
        sortBy: "createdAt",
    ...
    const [reviews, total] = await Promise.all([
        ...
        prisma_1.prisma.review.count({ where: { studentId: studentId } })
    ]);
    const totalPages = Math.ceil(total / paginationHelper.limit);
    return {
        data: reviews,
...
exports.StudentService = { updateProfile, getProfile, createReview, createBooking, getBookings, getReviewableBookings, cancelBooking, getReviews };
