...
const getAvailabilitySlots = async (req, res, next) => {
    try {
        ...
        const result = await TutorService.getAvailabilitySlots(req.user.id, weekStartDate);
        res.status(200).json({
            success: true,
...
export const updateAvailabilitySlots = async (req, res, next) => {
    try {
        // Validate the request body
        const validatedSlots = updateAvailabilitySlotsSchema.parse(req.body);
        // Update the availability slots
        const result = await TutorService.updateAvailabilitySlots(req.user.id, validatedSlots);
        res.status(200).json({
            success: true,
