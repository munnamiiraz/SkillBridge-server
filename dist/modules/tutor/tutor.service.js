...
exports.TutorService = exports.updateAvailabilitySlots = void 0;
// tutor.service.ts
...
const getDayOfWeek = (dateString) => new Date(dateString + "T00:00:00.000Z").getUTCDay();
/**
 ...
 */
const getAvailabilitySlots = async (userId, weekStartDate) => {
    const tutorProfile = await prisma_1.prisma.tutor_profile.findUnique({
        where: { userId }
    });
    if (!tutorProfile) {
    ...
    const slots = await prisma_1.prisma.availability_slot.findMany({
        where: {
    ...
    for (const s of slots) {
        if (!s.specificDate)
            continue;
        const dateKey = s.specificDate.toISOString().split('T')[0]; // YYYY-MM-DD
        if (!grouped[dateKey]) {
            grouped[dateKey] = [];
        }
...
const createProfile = async (userId, data) => {
    const validatedData = tutor_validation_1.createTutorProfileSchema.parse(data);
...
const updateProfile = async (userId, data) => {
    const validatedData = tutor_validation_1.updateTutorProfileSchema.parse(data);
    const updateData = {};
    ...
    return await prisma_1.prisma.tutor_profile.update({
        where: { userId },
        data: updateData
    });
};
const getProfile = async (userId) => {
    const profile = await prisma_1.prisma.tutor_profile.findUnique({
        where: { userId },
        include: {
            user: {
...
const getTeachingSessions = async (userId, options) => {
    const tutorProfile = await prisma_1.prisma.tutor_profile.findUnique({
        where: { userId }
    });
    if (!tutorProfile) {
...
const getReviews = async (userId, options) => {
    const tutorProfile = await prisma_1.prisma.tutor_profile.findUnique({
        where: { userId }
    });
    if (!tutorProfile) {
    ...
    const [reviews, total] = await Promise.all([
        ...
        prisma_1.prisma.review.count({ where: whereClause })
    ]);
    const totalPages = Math.ceil(total / paginationHelper.limit);
    return {
        data: reviews,
...
const updateBookingStatus = async (userId, bookingId, data) => {
    const tutorProfile = await prisma_1.prisma.tutor_profile.findUnique({
        where: { userId }
    });
    if (!tutorProfile) {
...
const updateAvailabilitySlots = async (userId, data) => {
    // ---------- helpers ----------
    // used global helpers: timeToMinutes, minutesToTime, getDayOfWeek, reverseDayOfWeekMap
    // ---------- tutor profile ----------
    const tutorProfile = await prisma_1.prisma.tutor_profile.findUnique({ where: { userId } });
    if (!tutorProfile)
        throw new Error("Tutor profile not found");
    // ---------- week bounds ----------
    ...
    for (const slot of data.slots) {
        ...
        if (isNaN(startMin) || isNaN(endMin))
            throw new Error(`Invalid time format for ${slot.date}`);
        if (endMin <= startMin)
            throw new Error(`Invalid time range for ${slot.date}: end must be after start`);
        if (endMin - startMin < 60)
            throw new Error(`Minimum duration is 1 hour for ${slot.date}`);
    ...
    const createdAndExisting = await prisma_1.prisma.$transaction(async (tx) => {
